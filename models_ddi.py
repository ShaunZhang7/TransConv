...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, **config):
        ...
        # self.icnn1 = nn.Conv2d(1, 3, 5, padding=0)
        ###self.icnn2 = nn.Conv2d(3, 6, 3, padding=0)
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        # repeat to have the same tensor size for aggregation shape都为torch.Size([16, 50, 545, 384])
        ...
        i = d_aug * p_aug  # interaction #shape:torch.Size([16, 50, 545, 384])
        # print(i.shape)
        ###i = abs(d_aug * d_aug - p_aug * p_aug)###i = (d_aug - p_aug) * (d_aug - p_aug)
        '''
        # 计算欧几里得距离的平方，形状保持为 torch.Size([16, 50, 545, 384])
        # 注意：这里我们计算的是每个对应元素之间的差的平方和，而不是真正的欧几里得距离（还需要开方,但为了保持形状，我们不开方）
        euclidean_distance_squared = (d_aug - p_aug) ** 2
        # 如果你想要真正的欧几里得距离（而不是平方），你可以在最后一个维度上开方，但这将改变张量的形状（减少一个维度），所以在这里我们不做这个操作
        i = euclidean_distance_squared
        
        # 重新整形为二维张量以计算余弦相似度
        # 形状变为 [16 * 50 * 545, 384]
        d_aug_flat = d_aug.view(-1, d_aug.size(-1))
        p_aug_flat = p_aug.view(-1, p_aug.size(-1))
        # 计算余弦相似度，形状为 [16 * 50 * 545]
        cos_similarity_flat = F.cosine_similarity(d_aug_flat, p_aug_flat, dim=-1)
        # 重新整形为原始形状 torch.Size([16, 50, 545])
        # 注意：这里我们丢失了一个维度（384），因为余弦相似度是一个标量值
        cos_similarity_reshaped = cos_similarity_flat.view(d_aug.size(0), d_aug.size(1), d_aug.size(2))
        # 为了保持形状 torch.Size([16, 50, 545, 384])，我们可以将相似度值重复384次
        # 但这通常不是计算余弦相似度的目的，因为余弦相似度本来就是一个标量值
        #i = cos_similarity_reshaped.unsqueeze(-1).repeat(1, 1, 1, 384)
        i_v = torch.unsqueeze(cos_similarity_reshaped, 1) # shape: torch.Size([16, 1, 50, 545])
        # print(i_v.shape)
        i_v = F.dropout(i_v, p=self.dropout_rate) # shape: torch.Size([16, 1, 50, 545])
        '''
        #d_aug = d_encoded_layers
        #p_aug = p_encoded_layers.permute(0, 2, 1)
        #i_v = torch.bmm(d_aug, p_aug)
        #'''
        ...
        i_v = torch.sum(i_v, dim=1) # shape: torch.Size([16, 50, 545])
        # print(i_v.shape)
        i_v = torch.unsqueeze(i_v, 1) # shape: torch.Size([16, 1, 50, 545])
        # print(i_v.shape)
        i_v = F.dropout(i_v, p=self.dropout_rate) # shape: torch.Size([16, 1, 50, 545])
        ...
        f = self.icnn(i_v) #############################################################################################
        #f = self.icnn(self.conv1x1(i_v).sum(dim=1, keepdim=True))  # 先使用1x1卷积提取特征，并通过求和/平均/最大的方式合并通道
        #print(f.shape) # =torch.Size([16, 3, 48, 543])
        ...
        score = self.decoder_1(f)  # shape: torch.Size([16, 3, 48, 543])
        '''
        decoder_feature = self.conv2d(i_v)
        #print(decoder_feature.shape) # torch.Size([16, 256, 3, 34]) ddi([16, 256, 3, 3])

        decoder_feature = decoder_feature.reshape(self.batch_size, 256, -1)
        #print(decoder_feature.shape) # torch.Size([16, 256, 102]) ddi([16, 256, 9])

        decoder_feature = self.conv1d(decoder_feature)
        #print(decoder_feature.shape) # torch.Size([16, 32, 11]) ddi([16, 32, 1])

        decoder_out = decoder_feature.reshape(self.batch_size, -1)
        #print(decoder_out.shape) # torch.Size([16, 352]) ddi([16, 32])

        score = self.fc(decoder_out)
        #'''
        '''
        decoder_feature = self.conv2d1(i_v)
        #print(decoder_feature.shape) # torch.Size([16, 512, 3, 34])
        decoder_feature = decoder_feature.reshape(self.batch_size, 256, -1)
        # print(decoder_feature.shape) # torch.Size([16, 512, 102])
        decoder_feature = self.conv1d1(decoder_feature)
        #print(decoder_feature.shape) # torch.Size([16, 32, 5])
        decoder_out = decoder_feature.reshape(self.batch_size, -1)
        #print(decoder_out.shape) # torch.Size([16, 32*5])
        score = self.fc1(decoder_out)
        '''
        return score
